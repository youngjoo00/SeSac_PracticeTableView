...
class MagazineInfoTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // DateFormatter() 를 이용하여 원하는 형식의 날짜 뽑아내기
        // 기존에 String 이었던 값을 -> Date -> String 순서로 변경한다.
        // dateFormat 의 형식에 맞춰서 값이 변하는 사실을 알게 되었다!
        
        cell.dateLabel.textColor = .lightGray
        cell.dateLabel.font = .boldSystemFont(ofSize: 15)
        let format = DateFormatter()
        format.dateFormat = "yyMMdd"
        if let date = format.date(from: magazine[indexPath.row].date) {
            format.dateFormat = "yy년 MM월 dd일"
            let dateString = format.string(from: date)
            cell.dateLabel.text = dateString
        } else {
            cell.dateLabel.text = "00년00월00일"
        }
